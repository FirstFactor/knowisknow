...
namespace BLL
{
    ...
    public partial class TopicUserView
    {
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Model.TopicUserView GetModelByCache(int topicID, int userID)
        {

            string CacheKey = "TopicUserViewModel-" + topicID + userID;
            object objModel = Maticsoft.Common.DataCache.GetCache(CacheKey);
            if (objModel == null)
            {
                try
                {
                    objModel = dal.GetModel(topicID, userID);
                    if (objModel != null)
                    {
            ...
            return (Model.TopicUserView)objModel;
        }
